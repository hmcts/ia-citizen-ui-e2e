...
export class ExuiLoginApi {
  ...
  public async login(username: string, password: string): Promise<void> {
    await expect(async () => {
      const startLoginResponse = await this.apiContext.get('auth/login');
      ...
      const hasAuthCookie = state.cookies.some((c) => c.name === '__auth__' || c.name === 'xui-webapp');

      if (!hasAuthCookie) {
        throw new Error('Login failed: Authentication cookies not found after redirect chain');
      }
    }).toPass({
      timeout: 30_000,
      intervals: [1_000],
