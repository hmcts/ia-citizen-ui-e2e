  ...
  public async submitEvent(options: RequestRespondentReviewEventType): Promise<void> {
    await expect(async () => {
      const triggerResponse = await this.apiContext.get(
        `data/internal/cases/${options.caseId}/event-triggers/requestRespondentReview?ignore-warning=false`,
        {
          headers: {
      ...
      for (const key of expectedKeys) {
        if (!(key in rawCaseData)) {
          throw new Error(`Critical Error: Expected field '${key}' was not found in the case record for event 'requestRespondentReview'.`);
        }
      }
      ...
      const submissionResponse = await this.apiContext.post(`data/cases/${options.caseId}/events`, {
        ...
        data: {
          data: finalData,
          event: { id: 'requestRespondentReview', summary: '', description: '' },
