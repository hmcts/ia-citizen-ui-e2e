...
import { RequestRespondentEvidenceEventType } from '../../../../exui-event-types';

export class RequestRespondentEvidenceApi {
  ...
  public async submitEvent(options: RequestRespondentEvidenceEventType): Promise<void> {
    await expect(async () => {
      const triggerResponse = await this.apiContext.get(
        `data/internal/cases/${options.caseId}/event-triggers/requestRespondentEvidence?ignore-warning=false`,
        {
          headers: {
      ...
      const submissionResponse = await this.apiContext.post(`data/cases/${options.caseId}/events`, {
        ...
        data: {
          data: finalData,
          event: { id: 'requestRespondentEvidence', summary: '', description: '' },
