...
import { RequestReasonsForAppealEventType } from '../../../../exui-event-types';

export class RequestReasonsForAppealApi {
  ...
  public async submitEvent(options: RequestReasonsForAppealEventType): Promise<void> {
    await expect(async () => {
      const triggerResponse = await this.apiContext.get(
        `data/internal/cases/${options.caseId}/event-triggers/requestReasonsForAppeal?ignore-warning=false`,
        {
          headers: {
      ...
      const expectedKeys = ['sendDirectionDateDue', 'sendDirectionExplanation', 'sendDirectionParties'];
      ...
      for (const key of expectedKeys) {
        if (!(key in rawCaseData)) {
          throw new Error(`Critical Error: Expected field '${key}' was not found in the case record for event 'requestReasonsForAppeal'.`);
        }
      }
      ...
      const submissionResponse = await this.apiContext.post(`data/cases/${options.caseId}/events`, {
        ...
        data: {
          data: finalData,
          event: { id: 'requestReasonsForAppeal', summary: '', description: '' },
