  ...
  public async submitEvent(options: UploadHomeOfficeBundleEventType): Promise<void> {
    await expect(async () => {
      const triggerResponse = await this.apiContext.get(
        `data/internal/cases/${options.caseId}/event-triggers/uploadHomeOfficeBundle?ignore-warning=false`,
        {
          headers: {
      ...
      const expectedKeys = ['uploadedHomeOfficeBundleDocs', 'homeOfficeBundle', 'appellantInDetention'];
      for (const key of expectedKeys) {
        if (!(key in rawCaseData)) {
          throw new Error(`Critical Error: Expected field '${key}' not found in case record.`);
        }
      }
      ...
      const submissionResponse = await this.apiContext.post(`data/cases/${options.caseId}/events`, {
        ...
        data: {
          data: finalData,
          event: { id: 'uploadHomeOfficeBundle', summary: '', description: '' },
