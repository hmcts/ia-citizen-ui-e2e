...
export class StartAppealAppealGroundsProtectionPage extends ExuiBase {
  ...
  public readonly $static = {
    ...
    appealGroundsAdviceText: this.page.locator('[id="appealGroundsProtectionTitle"] p'),
    selectAtLeastOneOptionHeading: this.page.getByRole('heading', {
      level: 2,
      name: 'Select at least one of the options below',
      exact: true,
    }),
    appealGroundsOptionLabels: this.page.locator('label[for^="appealGroundsProtection_values-"]'),
    humanRightsGroundsHeading: this.page.locator('dl[id="appealGroundsProtectionHumanRightsTitle"] h2'),
    optionalHumanRightsOptionHeading: this.page.getByRole('heading', {
      level: 2,
      name: 'Check the box if this statement also applies (Optional)',
      exact: true,
    }),
    humanRightsOptionLabel: this.page.locator('label[for="appealGroundsHumanRights_values-protectionHumanRights"]'),
  } as const satisfies Record<string, Locator>;

  public async verifyUserIsOnPage(): Promise<void> {
    await this.verifyUserIsOnExpectedPage({
      urlPath: 'startAppeal/startAppealappealGroundsProtection',
  ...
  public async completePageAndContinue(options: {
    groundsForAppeal: ExuiAppealGroundsProtectionType[];
    humanRightsGroundsForAppeal?: ExuiAppealGroundsProtectionHumanRightsType;
  }): Promise<void> {
