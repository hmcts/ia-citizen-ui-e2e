...
export class StartAppealOocHomeOfficeReferenceNumberPage extends ExuiBase {
  constructor(page: Page) {
    super(page);
  }

  public readonly $inputs = {
    homeOfficeReferenceNumberInput: this.page.locator('input[id="gwfReferenceNumber"]'),
  ...
  public readonly $static = {
    ...
    homeOfficeReferenceNumberLabel: this.page.locator('label[for="gwfReferenceNumber"] span'),
    homeOfficeReferenceNumberHint: this.page.locator('label[for="gwfReferenceNumber"] + span.form-hint'),
    adviceForEnteringCorrectNumberLabel: this.page.locator('[id="gwfReferenceNumberTips"] p'),
    adviceForEnteringCorrectNumberBulletPoint: this.page.locator('[id="gwfReferenceNumberTips"] ul li'),
  } as const satisfies Record<string, Locator>;

  public async verifyUserIsOnPage(): Promise<void> {
    await this.verifyUserIsOnExpectedPage({
      urlPath: 'startAppeal/startAppealoocHomeOfficeReferenceNumber',
  ...
  public async verifyAllTextOnPage(): Promise<void> {
    await Promise.all([
      ...
      expect(this.$static.homeOfficeReferenceNumberHint).toHaveText(
        'Enter the 16-digit UAN reference (four groups of four digits) or the 9-digit GWF reference (including the GWF prefix) in the decision letter',
      ),
