...
export class RequestRespondentEvidenceConfirmPage extends ExuiBase {
  ...
  public async verifyUserIsOnPage(): Promise<void> {
    await this.verifyUserIsOnExpectedPage({
      urlPath: 'trigger/requestRespondentEvidence/confirm',
  ...
  public async verifyAllTextOnPage(): Promise<void> {
    await Promise.all([
      ...
      expect(this.$static.whatHappensNextParagraph1).toHaveText(
        'Wait for the respondent to complete the direction. You must upload the respondent evidence as soon as you receive it.',
      ),
    ]);
  }
