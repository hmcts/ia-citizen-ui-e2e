...
export class HearingLoopPage extends CuiBase {
  ...
  private readonly pageForm = this.page.locator('body:has(form[action="/hearing-hearing-loop"])');
  ...
  public readonly $static = {
    pageHeading: this.pageForm.getByRole('heading', { name: 'Will you or any witnesses need a hearing loop?', level: 1, exact: true }),
  ...
  public async completePageAndContinue(option: { willYouOrWitnessNeedHearingLoop: YesOrNoType; verifyAllTextOnPage?: boolean }): Promise<void> {
    ...
    const element = this.pageForm.locator(`input[type="radio"][value="${option.willYouOrWitnessNeedHearingLoop.toLowerCase()}"]`);
    await element.check();
