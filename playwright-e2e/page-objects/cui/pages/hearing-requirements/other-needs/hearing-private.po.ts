...
export class HearingPrivatePage extends CuiBase {
  constructor(page: Page) {
    super(page);
  }

  private readonly pageForm = this.page.locator('body:has(form[action="/hearing-private"])');
  ...
  public readonly $static = {
    pageHeading: this.pageForm.getByRole('heading', { name: 'Will you need a private hearing?', level: 1, exact: true }),
    privateHearingHintText: this.pageForm.locator('div[id="answer-hint"]'),
  ...
  public async completePageAndContinue(option: { willYouNeedAPrivateHearing: YesOrNoType; verifyAllTextOnPage?: boolean }): Promise<void> {
    ...
    const element = this.pageForm.locator(`input[type="radio"][value="${option.willYouNeedAPrivateHearing.toLowerCase()}"]`);
    await element.check();
