...
export class HearingMultimediaEvidencePage extends CuiBase {
  ...
  private readonly pageForm = this.page.locator('body:has(form[action="/hearing-multimedia-evidence"])');
  ...
  public readonly $static = {
    pageHeading: this.pageForm.getByRole('heading', { name: 'Will you bring any video or audio evidence to the hearing?', level: 1, exact: true }),
    videoAndAudioEvidenceHintText: this.pageForm.locator('div[id="answer-hint"]'),
  ...
  public async completePageAndContinue(option: { willYouBringVideoOrAudioEvidence: YesOrNoType; verifyAllTextOnPage?: boolean }): Promise<void> {
    ...
    const element = this.pageForm.locator(`input[type="radio"][value="${option.willYouBringVideoOrAudioEvidence.toLowerCase()}"]`);
    await element.check();
