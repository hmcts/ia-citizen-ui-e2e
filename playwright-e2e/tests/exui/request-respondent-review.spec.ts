  ...
  test('Verify case officer is able to request respondent review', async ({ exui_pages, dataUtils }) => {
    await test.step('Verify correct next steps are displayed on case overview page', async () => {
      await Promise.all([
        ...
        expect(exui_pages.caseOverviewPage.$static.doThisNextParagraph.nth(0)).toHaveText("Review the appellant's case in the appeal tab."),
        ...
        expect(exui_pages.caseOverviewPage.$static.doThisNextParagraph.nth(3)).toHaveText('You can do this from the directions tab.'),
    ...
    await test.step('Select request respondent review from next steps dropdown and submit event', async () => {
      await exui_pages.caseOverviewPage.selectEventFromDropdown({ eventToSelect: 'Request respondent review' });

      await exui_pages.requestRespondentReviewPage.verifyUserIsOnPage();
      await exui_pages.requestRespondentReviewPage.verifyAllTextOnPage();
      await exui_pages.requestRespondentReviewPage.continueOnToNextPage();

      await exui_pages.requestRespondentReviewSubmitPage.verifyUserIsOnPage();
      ...
      await Promise.all([
        expect(exui_pages.requestRespondentReviewSubmitPage.$static.caseRecordHeading).toBeVisible(),
        expect(exui_pages.requestRespondentReviewSubmitPage.$static.checkYouAnswersHeading).toBeVisible(),
        expect(exui_pages.requestRespondentReviewSubmitPage.$static.checkInformationCarefullyText).toBeVisible(),

        expect(exui_pages.requestRespondentReviewSubmitPage.$static.directionYouAreIssuingQuestion).toHaveText(
          'Explain the direction you are issuing',
        ),
        expect(exui_pages.requestRespondentReviewSubmitPage.$static.directionYouAreIssuingQuestion).toBeVisible(),
        expect(exui_pages.requestRespondentReviewSubmitPage.$static.directionYouAreIssuingValue)
      ...
      Parties must ensure they conduct proceedings with procedural rigour. The Tribunal will not overlook breaches of the requirements of the Procedure Rules, Practice Statement or Practice Direction, nor failures to comply with directions issued by the Tribunal. Parties are reminded of the sanctions for non-compliance set out in paragraph 5.3 of the Practice Direction of 01.11.24.`),
        expect(exui_pages.requestRespondentReviewSubmitPage.$static.directionYouAreIssuingValue).toBeVisible(),

        expect(exui_pages.requestRespondentReviewSubmitPage.$static.whoAreYouGivingDirectionToQuestion).toHaveText(
          'Who are you giving the direction to?',
        ),
        expect(exui_pages.requestRespondentReviewSubmitPage.$static.whoAreYouGivingDirectionToQuestion).toBeVisible(),
        expect(exui_pages.requestRespondentReviewSubmitPage.$static.whoAreYouGivingDirectionToValue).toHaveText('Respondent'),
        expect(exui_pages.requestRespondentReviewSubmitPage.$static.whoAreYouGivingDirectionToValue).toBeVisible(),

        expect(exui_pages.requestRespondentReviewSubmitPage.$static.byWhatDateMustTheyComplyQuestion).toHaveText('By what date must they comply?'),
        expect(exui_pages.requestRespondentReviewSubmitPage.$static.byWhatDateMustTheyComplyQuestion).toBeVisible(),
        expect(exui_pages.requestRespondentReviewSubmitPage.$static.byWhatDateMustTheyComplyValue).toHaveText(formattedExpectedDate),
        expect(exui_pages.requestRespondentReviewSubmitPage.$static.byWhatDateMustTheyComplyValue).toBeVisible(),

        expect(exui_pages.requestRespondentReviewSubmitPage.$interactive.changeDirectionYouAreIssuingButton).toHaveText('Change'),
        expect(exui_pages.requestRespondentReviewSubmitPage.$interactive.changeDirectionYouAreIssuingButton).toBeVisible(),
        expect(exui_pages.requestRespondentReviewSubmitPage.$interactive.changeByWhatDateMustTheyComplyButton).toHaveText('Change'),
        expect(exui_pages.requestRespondentReviewSubmitPage.$interactive.changeByWhatDateMustTheyComplyButton).toBeVisible(),
      ]);

      await exui_pages.requestRespondentReviewSubmitPage.sendDirection();

      await exui_pages.requestRespondentReviewConfirmPage.verifyUserIsOnPage();
      await exui_pages.requestRespondentReviewConfirmPage.verifyAllTextOnPage();
      await exui_pages.requestRespondentReviewConfirmPage.returnToCaseDetails();
    });
    ...
    await test.step('Verify correct next steps are displayed once event has been submitted', async () => {
      ...
      await exui_pages.caseOverviewPage.verifyAlertMessageAfterSubmittingEvent({ eventSubmitted: 'Request respondent review' });
