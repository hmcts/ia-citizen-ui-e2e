  ...
  test('Verify case officer is able to request aip appeal reasons', async ({ exui_pages, dataUtils }) => {
    await test.step('Verify correct next steps are displayed on case overview page', async () => {
      await Promise.all([
        ...
        expect(exui_pages.caseOverviewPage.$static.doThisNextParagraph.nth(0)).toHaveText('The respondent has submitted their evidence.'),
    ...
    await test.step('Select Aip - request appeal reasons from next steps dropdown and submit event', async () => {
      await exui_pages.caseOverviewPage.selectEventFromDropdown({ eventToSelect: 'AiP - Request Appeal Reasons' });

      await exui_pages.aipRequestAppealReasonsPage.verifyUserIsOnPage();
      await exui_pages.aipRequestAppealReasonsPage.verifyAllTextOnPage();
      await exui_pages.aipRequestAppealReasonsPage.continueOnToNextPage();

      await exui_pages.aipRequestAppealReasonsSubmitPage.verifyUserIsOnPage();
      ...
      await Promise.all([
        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.caseRecordHeading).toBeVisible(),
        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.checkYouAnswersHeading).toBeVisible(),
        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.checkInformationCarefullyText).toBeVisible(),

        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.directionYouAreIssuingQuestion).toHaveText(
          'Explain the direction you are issuing',
        ),
        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.directionYouAreIssuingQuestion).toBeVisible(),
        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.directionYouAreIssuingValue).toHaveText(
          'You must now tell us why you think the Home Office decision to refuse your claim is wrong.',
        ),
        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.directionYouAreIssuingValue).toBeVisible(),

        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.whoAreYouGivingDirectionToQuestion).toHaveText(
          'Who are you giving the direction to?',
        ),
        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.whoAreYouGivingDirectionToQuestion).toBeVisible(),
        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.whoAreYouGivingDirectionToValue).toHaveText('Appellant'),
        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.whoAreYouGivingDirectionToValue).toBeVisible(),

        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.byWhatDateMustTheyComplyQuestion).toHaveText('By what date must they comply?'),
        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.byWhatDateMustTheyComplyQuestion).toBeVisible(),
        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.byWhatDateMustTheyComplyValue).toHaveText(formattedExpectedDate),
        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$static.byWhatDateMustTheyComplyValue).toBeVisible(),

        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$interactive.changeByWhatDateMustTheyComplyButton).toHaveText('Change'),
        expect(exui_pages.aipRequestAppealReasonsSubmitPage.$interactive.changeByWhatDateMustTheyComplyButton).toBeVisible(),
      ]);

      await exui_pages.aipRequestAppealReasonsSubmitPage.sendDirection();

      await exui_pages.aipRequestAppealReasonsConfirmPage.verifyUserIsOnPage();
      await exui_pages.aipRequestAppealReasonsConfirmPage.verifyAllTextOnPage();
      await exui_pages.aipRequestAppealReasonsConfirmPage.returnToCaseDetails();
    });
    ...
    await test.step('Verify correct next steps are displayed once event has been submitted', async () => {
      ...
      await exui_pages.caseOverviewPage.verifyAlertMessageAfterSubmittingEvent({ eventSubmitted: 'AiP - Request Appeal Reasons' });
